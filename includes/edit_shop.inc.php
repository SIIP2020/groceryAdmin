...
{    
 ...
 // Check extension
       if( in_array($imageFileType,$extensions_arr) ){                  
         ...
         $sql = "UPDATE shop SET shop_name=? , shop_image=?  , open_time=? , close_time=? , status=? , street_address=? , city=? , state=? , pincode=? , gst_no=? , license_no=?  where shop_id=$sid ";
         ...
         $stmt = mysqli_stmt_init($conn);
            ...
            else
            {
                ...
                header("Location: ../edit_shop.php?shop_id=".base64_encode($sid));
                exit();
            }
